...
pub mod farming {
  ...
  pub fn close(_ctx: Context<CloseFarming>) -> Result<()> {
    Ok(())
  }

  pub fn decrement(ctx: Context<Update>) -> Result<()> {
    ctx.accounts.farming.count = ctx.accounts.farming.count.checked_sub(1).unwrap();
    Ok(())
  }

  pub fn increment(ctx: Context<Update>) -> Result<()> {
    ctx.accounts.farming.count = ctx.accounts.farming.count.checked_add(1).unwrap();
    Ok(())
  }

  pub fn initialize(_ctx: Context<InitializeFarming>) -> Result<()> {
    Ok(())
  }

  pub fn set(ctx: Context<Update>, value: u8) -> Result<()> {
    ctx.accounts.farming.count = value.clone();
    Ok(())
  }
  ///Charge Reward
...
pub struct InitializeFarming<'info> {
  ...
  #[account(
  init,
  space = 8 + Farming::INIT_SPACE,
  payer = payer
  )]
  pub farming: Account<'info, Farming>,
  pub system_program: Program<'info, System>,
}
#[derive(Accounts)]
pub struct CloseFarming<'info> {
  #[account(mut)]
  pub payer: Signer<'info>,
